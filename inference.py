...
def initialize_pipeline(config):
    ...
    if config.noise_rectification_flag:
        ...
        pipe.scheduler.set_timesteps(config.n_steps)
    ...
    with torch.autocast(config.device, dtype=torch.half):
        ...
        for i in range(config.repeat_num):
            ...
            with torch.no_grad():
                ...
                else: # for 'InjectionPipeline' and  'Text2VideoSDPipeline'
                    print("prompt type check :", config.prompt)
                    video_frames = pipe(
                        prompt=config.prompt, ##error
                        negative_prompt=config.negative_prompt,
                        width=config.width,
                        height=config.height,
                        num_frames=config.num_frames,
...
if __name__ == "__main__":
    ...
    # config.prompt = [prompt] * config.batch_size
